...
public class TestStrategy1WithTrailSL25 {
...
public static void main(String[] args) throws Exception {
	...
	private static void analyzeSymbol(String symbol, LocalDate targetDay,
		...
		// --- First 3 candles of targetDay: compute buyPrice and candleRange ---
		...
		double candleRange     = buyPrice - lowestLow;          // high - low of first 3 candles
		double initialStopLoss = buyPrice - 0.25 * candleRange; // SL = buyPrice - 25% of range
		double stopLoss        = initialStopLoss;
		double target          = 2 * buyPrice - initialStopLoss; // 1:1 risk-reward based on new SL
		...
		for (int j = startIdx + 3; j < candles.size(); j++) {
			...
			if (!triggered) {
				...
				if (c.high > buyPrice) {
					...
					status        = "Open";
					trailingHigh  = c.high;
					// TrailSL: initialSL + 1 * (newHigh - buyPrice) — moves 1:1 with each new high
					stopLoss = initialStopLoss + (trailingHigh - buyPrice);
				...
				if (c.high > trailingHigh) {
					trailingHigh = c.high;
					// TrailSL: initialSL + 1 * (newHigh - buyPrice) — moves 1:1
					stopLoss = initialStopLoss + (trailingHigh - buyPrice);
				}
